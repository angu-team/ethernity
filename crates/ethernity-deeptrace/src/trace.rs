    ...
    fn build_node(trace: &CallTrace, depth: usize, index: &mut usize) -> Result<CallNode> {
        ...
        *index += 1;
        ...
        let from = Address::from_str(&trace.from.trim_start_matches("0x"))
            .map_err(|_| Error::DecodeError(format!("Endereço inválido: {}", trace.from)))?;
        
        let to = if trace.to.is_empty() {
            None
        } else {
            Some(Address::from_str(&trace.to.trim_start_matches("0x"))
                .map_err(|_| Error::DecodeError(format!("Endereço inválido: {}", trace.to)))?)
        };
        
        let value = U256::from_dec_str(&trace.value)
            .map_err(|_| Error::DecodeError(format!("Valor inválido: {}", trace.value)))?;
        
        let gas = U256::from_dec_str(&trace.gas)
            .map_err(|_| Error::DecodeError(format!("Gas inválido: {}", trace.gas)))?;
        
        let gas_used = U256::from_dec_str(&trace.gasUsed)
            .map_err(|_| Error::DecodeError(format!("Gas usado inválido: {}", trace.gasUsed)))?;
        
        let input = hex::decode(trace.input.trim_start_matches("0x"))
            .map_err(|_| Error::DecodeError(format!("Input inválido: {}", trace.input)))?;
        
        let output = hex::decode(trace.output.trim_start_matches("0x"))
            .map_err(|_| Error::DecodeError(format!("Output inválido: {}", trace.output)))?;
        
        let call_type = trace.type_.as_deref().map(CallType::from).unwrap_or(CallType::Call);
